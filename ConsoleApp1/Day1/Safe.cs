...
R14";
    ...
    {
        Console.Write($"Dial at {DialPosition}, to ");

        int netClicks = clicks%100;
        // Add full spins touching 0
        CountTouchedZero += clicks / 100;
        switch (direction)
        {
            case 'L':
                Console.Write($"Left {clicks}");
                if (clicks >= 100)
                {
                    Console.Write($"({netClicks}) ");
                }
                if (DialPosition > 0 && DialPosition - netClicks <= 0)
                {
                    Console.Write("+ ZERO ");
                    CountTouchedZero++;
                }
                // X moves to the left is equivalent to (100 - X) moves to the right on ending position        
                //Console.Write($" AM:{netClicks}, ");
                DialPosition += (100 - netClicks);
                break;
            case 'R':
                Console.Write($"Right {clicks}");
                if (clicks >= 100)
                {
                    Console.Write($"({netClicks}) ");
                }
                if (DialPosition + netClicks >= 100)
                {
                    Console.Write("+ ZERO ");
                    CountTouchedZero++;
                }
        ...
        if (clicks >= 100)
        {
            Console.Write($" + {clicks / 100} * ZERO (Full Spin) ");
        }

        //Console.Write($"{netClicks}, ");
        DialPosition %= 100;
        Console.Write($" > Dial ended in {DialPosition}!");
        if (DialPosition == 0)
        {
            CountLandedAtZero++;
            //Console.WriteLine();
